document.addEventListener('DOMContentLoaded', () => {
	...
	apiKeyInput.addEventListener('input', (event) => {
		if (event.target.value.length === 32) {
			...
			if (event_properties['invalid input']) {
				delete event_properties['invalid input']
			}
	...
	const postRequest = (url, data) => {
	  ...
	  // If all form data is entered in, the user can send the event to Amplitude. 
	  const sendEvent = () => {
		  ...
		  if (isFilledOut) {
			  ...
			  event_properties["deviceID"] = customDeviceId.value || "accounts-validation-tool"
			  event_properties["eventType"] = event
			  event_properties["groupType"] = group
			  event_properties["groupNames"] = groupNames
			  amp.track("Filled Event Form", event_properties)
			  ...
			  if (event && group && groupNames) {
				  sendEventButton.addEventListener("click", () => {
					  ...
					  // event_properties["groupType"] = group
					  // event_properties["groupNames"] = groupNames
					  amp.track("Sent Event", event_properties)
				  })
			  }
	  ...
	  const groupPropsInput = () => {
		  ...
		  try {
			  ...
			  if (inputJSON && JSON.stringify(groupsInfo) && groupType.value.length != 0 && groupValue.value.length != 0) {
				  ...
				  if (propsCodeBlock.textContent && API_KEY) {
					  sendGroupPropsButton.addEventListener("click", () => {
						  ...
						  event_properties["deviceID"] = customDeviceId.value || "accounts-validation-tool"
						  event_properties["eventType"] = eventName.value
						  event_properties["groupProperties"] = groupsInfoString
						  amp.track("Sent Group Properties", event_properties)
						  }
					  );
				  }
